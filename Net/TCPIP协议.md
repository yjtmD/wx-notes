...
__不难看出，TCP/IP 与 OSI 在分层模块上稍有区别。OSI 参考模型注重“通信协议必要的功能是什么”，而 TCP/IP 则更强调“在计算机上实现协议应该开发哪种程序”。__
...
* ① 和 ② 的通信是在一台计算机上进行的。它们的目标端口号相同，都是80。这里可以根据源端口号加以区分。
* ③ 和 ① 的目标端口号和源端口号完全相同，但它们各自的源 IP 地址不同。
* 此外，当 IP 地址和端口号全都一样时，我们还可以通过协议号来区分（TCP 和 UDP）。

> 同一个浏览器的多个页面在建立TCP链接的时候，会随机启用一个端口号，即上述源端口号，链接关闭后，端口号释放，可被其他页面复用。
...
__上面是一方主动关闭，另一方被动关闭的情况，实际中还会出现同时发起主动关闭的情况，__
具体流程如下图：
